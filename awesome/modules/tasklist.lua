--- Patched version of default Awesome tasklist widget
--- Patched to allow fullscreen_desktop windows to exist in the tasklist

print("STELLAR: Patched Awesome tasklist widget loaded.")
---------------------------------------------------------------------------
...
local function tasklist_update(s, self, buttons, filter, data, style, update_function, args)
    ...
    for _, c in ipairs(list) do
        if not (c.skip_taskbar
            or (c.hidden and not c._stellar_show_in_tasklist)
            or c.type == "splash" or c.type == "dock"
            or (c.type == "desktop" and not c._stellar_show_in_tasklist))
            and filter(c, s) then
